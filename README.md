...
- at least 8GB RAM for llama3 / 16GB RAM for gemma4:e4b
...
# 4. Install Ollama and get LLM (e.g. gemma4:e4b)
...
1. **watchdog** listens for two events in `WATCH_DIR`:
...
4. **Content extractor** reads a text snippet from the file:
   ...
   - `.pptx` — analyzed via python-pptx
   - `.jpg`, `.png`, … — analyzed via Pillow
   - `.txt`, `.md`, `.py`, `.html`, … — direct read
   - Unreadable files (e.g. binaries) — filename only
5. **Ollama classifier** receives the text snippet + your real folder tree
   (scanned live from `TARGET_DIR`) and returns:
   - `target_path` — exact relative folder name based on folders in `TARGET_DIR`
   - `confidence` — float 0–1
   - `reason` — one-sentence explanation
6. **Executor** moves the file to `target_path/filename`,
...
| `TARGET_DIR` | `~/Desktop/University` | Root of your lecture folder tree |
| `LOG_DIR` | `./log` | Root of your log files
| `OLLAMA_MODEL` | `"gemma4:e4b"` | Any model you have pulled locally |
| `OLLAMA_URL` | `http://localhost:11434` | Ollama server address |
| `MAX_CONTENT_CHARS` | `3000` | Text chars sent to the LLM per file |
| `CONFIDENCE_THRESHOLD` | `0.81` | Below this → file is not moved |
| `MIN_FILE_AGE_SECONDS` | `2` | Wait time after file appears |
| `SUMMARY_HOUR` | `18` | Hour for the automatic daily summary |

The agent scans `TARGET_DIR` at startup and picks up new folders
...
- **Self-correction** — low-confidence results are not moved
